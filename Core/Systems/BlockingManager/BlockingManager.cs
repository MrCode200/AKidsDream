...
[Icon("res://Core/Systems/BlockingManager/do-not-touch.svg")]
public partial class BlockingManager : Node
{
	...
	private void OnEffectTriggerEnd(Node _, AbilityData __, EffectData effectData)
	{
		if (effectData.BlockOnTrigger)
	...
	{
		foreach (var strategy in blockable.BlockingStrategies)
		{
	...
	{
		bool shouldBlock = obj.BlockingStrategies.Any(strat => _activeCounts.GetValueOrDefault(strat) > 0);
