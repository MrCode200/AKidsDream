...
4. Clicking empty board does nothing.
...
*/
// TODO:
// check what should become a command and what not
...
public sealed class PlayerInteractionPayload(
    ...
    TileData? tileAtMousePos,
    bool isLeftClickPressed
)
{
    public readonly InputEvent InputEvent = inputEvent;
    public readonly TileData? TileAtMousePos = tileAtMousePos;
...
{
    [Export] public required Visualizer AbilityVisualizer;
    [Export] public required StateMachine StateMachine;
    [Export] public required CommandExecutor CommandExecutor;
...
public class OnAbilitySelected(PlayerInteractionController pic) : IState
{
    ...
    private void HandleHover(PlayerInteractionPayload interaction)
    {
        if (!IsTileInsideReach(interaction.TileLocationAtMousePos))
        {
            // is it needed to show current targets only here too? why redraw if not needed right?
            return;
        }
